# Implementation of classic arcade game Pong

import simplegui
import random

# initialize globals - pos and vel encode vertical info for paddles
WIDTH = 600
HEIGHT = 400
BALL_RADIUS = 20
PAD_WIDTH = 8
PAD_HEIGHT = 80
HALF_PAD_WIDTH = PAD_WIDTH / 2
HALF_PAD_HEIGHT = PAD_HEIGHT / 2
LEFT = False
RIGHT = True
PAD_VEL = 5

def spawn_ball(direction):
    """Initialize ball_pos and ball_vel for new bal in middle of table"""
    global ball_pos, ball_vel  # these are vectors stored as lists
    ball_pos = [WIDTH / 2, HEIGHT / 2]
    ball_vel = [0, 0]
    # randomization of]] velocity
    # if direction is RIGHT, the ball's velocity is upper right,
    # else upper left
    if direction == RIGHT:
        ball_vel[0] = 1 * random.randrange(120, 140) / 60
        ball_vel[1] = -1 * random.randrange(60, 180) / 60
    if direction == LEFT:
        ball_vel[0] = -1 * random.randrange(120, 140) / 60
        ball_vel[1] = -1 * random.randrange(60, 180) / 60


# define event handlers
def new_game():
    """Start new game"""
    global paddle1_pos, paddle2_pos, paddle1_vel, paddle2_vel  # these are numbers
    global score1, score2  # these are ints
    # spawn ball
    spawn_ball(LEFT)
    # set position and velocity of paddles
    paddle1_pos = HEIGHT / 2 + HALF_PAD_HEIGHT
    paddle2_pos = HEIGHT / 2 + HALF_PAD_HEIGHT
    paddle1_vel = 0
    paddle2_vel = 0
    # init game scores
    score1 = 0
    score2 = 0


def draw(canvas):
    """Update ball and paddles positions"""
    global score1, score2, paddle1_pos, paddle2_pos, ball_pos, ball_vel

    # draw mid line and gutters
    canvas.draw_line([WIDTH / 2, 0],[WIDTH / 2, HEIGHT], 1, "White")
    canvas.draw_line([PAD_WIDTH, 0],[PAD_WIDTH, HEIGHT], 1, "White")
    canvas.draw_line([WIDTH - PAD_WIDTH, 0],[WIDTH - PAD_WIDTH, HEIGHT], 1, "White")
    
    # update ball
    ball_pos[0] += ball_vel[0]
    ball_pos[1] += ball_vel[1]

    # law of ball reflection
    if ball_pos[1] <= BALL_RADIUS:
        ball_vel[1] = - ball_vel[1]
    if ball_pos[1] >= HEIGHT - BALL_RADIUS:
        ball_vel[1] = - ball_vel[1]
   
    # determine whether left paddle and ball collide
    if ball_pos[0] <= BALL_RADIUS + PAD_WIDTH:
        if ball_pos[1] <= paddle1_pos and ball_pos[1] >= paddle1_pos - PAD_HEIGHT:
            ball_vel[0] = - ball_vel[0]
            # increase ball spead
            ball_vel[0] += ball_vel[0] * 0.1
            ball_vel[1] += ball_vel[1] * 0.1
        else:
            spawn_ball(RIGHT)
            score2 += 1

    # determine whether right paddle and ball collide
    if ball_pos[0] >= WIDTH - BALL_RADIUS - PAD_WIDTH:
        if ball_pos[1] <= paddle2_pos and ball_pos[1] >= paddle2_pos - PAD_HEIGHT:
            ball_vel[0] = - ball_vel[0]
            # increase ball spead
            ball_vel[0] += ball_vel[0] * 0.1
            ball_vel[1] += ball_vel[1] * 0.1
        else:
            spawn_ball(LEFT)
            score1 += 1

    # draw ball
    canvas.draw_circle(ball_pos, BALL_RADIUS, 2, "Red", "White")
    
    # update paddle's vertical position, keep paddle on the screen
    if paddle1_pos - PAD_HEIGHT + paddle1_vel >= 0 and paddle1_pos + paddle1_vel <= HEIGHT:
        paddle1_pos += paddle1_vel
    if paddle2_pos - PAD_HEIGHT + paddle2_vel >= 0 and paddle2_pos + paddle2_vel <= HEIGHT:
        paddle2_pos += paddle2_vel

    # draw paddles
    canvas.draw_polygon([[0, paddle1_pos], [PAD_WIDTH, paddle1_pos], [PAD_WIDTH, paddle1_pos - PAD_HEIGHT],
                         [0, paddle1_pos - PAD_HEIGHT]], 3, 'White', 'White')
    canvas.draw_polygon([[WIDTH-PAD_WIDTH, paddle2_pos], [WIDTH, paddle2_pos], [WIDTH, paddle2_pos - PAD_HEIGHT],
                         [WIDTH-PAD_WIDTH, paddle2_pos - PAD_HEIGHT]], 3, 'White', 'White')

    # draw scores
    canvas.draw_text(str(score1), (200, 50), 25, "Red")
    canvas.draw_text(str(score2), (400, 50), 25, "Red")


def keydown(key):
    global paddle1_vel, paddle2_vel

    # for paddle 1
    if key == simplegui.KEY_MAP["w"]:
        paddle1_vel = -PAD_VEL
    elif key == simplegui.KEY_MAP["s"]:
        paddle1_vel = PAD_VEL

    # for paddle 2
    if key == simplegui.KEY_MAP["up"]:
        paddle2_vel = -PAD_VEL
    elif key == simplegui.KEY_MAP["down"]:
        paddle2_vel = PAD_VEL


def keyup(key):
    global paddle1_vel, paddle2_vel

    # for paddle 1
    if key == simplegui.KEY_MAP["w"]:
        paddle1_vel = 0
    elif key == simplegui.KEY_MAP["s"]:
        paddle1_vel = 0

    # for paddle 2
    if key == simplegui.KEY_MAP["up"]:
        paddle2_vel = 0
    elif key == simplegui.KEY_MAP["down"]:
        paddle2_vel = 0


# create frame
frame = simplegui.create_frame("Pong", WIDTH, HEIGHT)
frame.set_draw_handler(draw)
frame.set_keydown_handler(keydown)
frame.set_keyup_handler(keyup)
frame.add_button("restart", new_game, 200)

# start frame
new_game()
frame.start()
